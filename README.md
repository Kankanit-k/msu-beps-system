...
| อะไร                    | URL                                  | หมายเหตุ                                                                                                                                                                      |
| ----------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| แอปจริง (Next.js + MUI) | <https://msu-beps-web.vercel.app>    | ครบ 24 หน้าจอ · **ปิดการตรวจสิทธิ์ไว้** (`AUTH_DISABLED=true`) เพราะ ERP ยังไม่ได้ลงทะเบียน callback ของโดเมนนี้ — ทุกหน้าจึงเปิดได้โดยไม่ต้องล็อกอิน · ใส่ `noindex` ไว้แล้ว |
| mockup HTML เดิม        | <https://msu-beps-mockup.vercel.app> | เก็บไว้เป็นต้นฉบับอ้างอิง ไม่ได้แตะ                                                                                                                                           |

ทั้งสองเว็บใช้**ชุดข้อมูลตัวอย่าง** ไม่ใช่ตัวเลขจริงของมหาวิทยาลัย (ทุกหน้ามีแถบแดงกำกับไว้)

deploy แอปใหม่: `npx vercel deploy --prod` ที่รากโปรเจกต์ (โปรเจกต์ `msu-beps-web` ตั้ง Root Directory
ไว้ที่ `apps/web` แล้ว ส่วน `vercel.json` ที่รากเป็นของโปรเจกต์ mockup — คนละโปรเจกต์กัน)

> **ก่อนเปิดใช้จริงต้องทำ** — ให้ทีม ERP ลงทะเบียน callback `<โดเมน>/beps/api/auth/callback/erpauth`
> แล้วตั้ง `AUTH_DISABLED=false` + `NEXT_PUBLIC_AUTH_DISABLED=false` + `AUTH_CLIENT_ID/SECRET` บน Vercel

### เอกสารสำหรับนำไปประชุม

**[`mockup/`](./mockup/)** — ต้นแบบหน้าจอครบทั้งระบบ 20 หน้า (W0–W19) คลิกได้จริง ใช้สูตรและข้อมูลจริงปี 2568
ดูออนไลน์ที่ <https://msu-beps-mockup.vercel.app> หรือเปิดจากเครื่องที่ [`mockup/index.html`](./mockup/index.html)
· ใช้เสนอผู้บริหารและส่งต่อ dev ได้ทีละหน้า
