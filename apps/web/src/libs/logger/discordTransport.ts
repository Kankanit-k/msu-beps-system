...
const buildPayload = (entry: LogEntry) => {
  ...
  if (entry.error?.message) {
    fields.push({
      name: 'Error',
      value: clamp(`**${entry.error.name}**: ${entry.error.message}`, 1024),
    });
  }
  ...
  if (entry.context && Object.keys(entry.context).length > 0) {
    fields.push({
      name: 'Context',
      value: codeBlock(JSON.stringify(entry.context, null, 2), 1024),
    });
  }
  ...
  if (suppressed > 0) {
    fields.push({
      name: 'Suppressed',
      value: `${suppressed} alert(s) rate-limited in this window`,
    });
  }
  ...
  return {
    ...
    content: clamp(
      `${mention}${icons[entry.level]} **${entry.level.toUpperCase()}** — ${entry.message}`,
      2000,
    ),
    embeds: [
      {
  ...
  try {
    ...
    if (res.status === 429 && attempt < 2) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, Math.min(retryAfter * 1000 || 1000, 5000)),
      );
...
export const isDiscordConfigured = () =>
  Boolean(loggerConfig.discord.url && loggerConfig.discord.enabled);
