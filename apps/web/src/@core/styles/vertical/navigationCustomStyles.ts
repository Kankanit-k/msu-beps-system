...
const navigationCustomStyles = (verticalNavOptions: VerticalNavState, theme: Theme) => {
  ...
  return {
    ...
    [`& .${verticalNavClasses.container}`]: {
      ...
      // Visible right border + soft shadow so the sidebar reads as a framed panel
      borderColor: 'var(--mui-palette-divider)',
      boxShadow:
        theme.direction === 'rtl'
          ? '-4px 0 16px -10px rgba(15, 23, 42, 0.10)'
          : '4px 0 16px -10px rgba(15, 23, 42, 0.10)',
      ...(collapsedHovered && {
    ...
    [`& .${menuClasses.root}`]: {
      ...
      // Left/right gutter so items aren't flush against the sidebar edges
      ...(collapsedNotHovered
        ? {
