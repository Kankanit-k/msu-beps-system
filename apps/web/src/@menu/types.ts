...
export type SubMenuItemElement =
  'root' | 'button' | 'label' | 'prefix' | 'suffix' | 'icon' | 'subMenuStyles' | 'subMenuContent' | 'subMenuExpandIcon'
...
export type RenderExpandedMenuItemIcon = {
  icon:
    ReactElement | ((params: { level?: number; active?: boolean; disabled?: boolean }) => ReactElement | null) | null
  level?: number
}
