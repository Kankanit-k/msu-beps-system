...
export async function register() {
  ...
  const { logger, isDiscordConfigured, currentLogFiles, closeFiles } =
    await import('@/libs/logger');
...
export async function onRequestError(
  ...
  logger.error(
    `Request failed: ${request.method ?? 'GET'} ${request.path ?? context.routePath ?? 'unknown'}`,
    {
